...
# Given integer x, this returns the integer floor(root(x)).
def root(x):
	assert x >= 0
	i = 1
	while i * i <= x:
...
def findPrimes(n):
	return [i for (i, isprime) in enumerate(prime_check(n)) if isprime]


# Yields prime numbers in ascending order from 2 to ceiling (inclusive).
def prime_generator(ceiling):
	if ceiling >= 2:
		yield 2
	
	# Sieve of Eratosthenes, storing only odd numbers starting at 3
	isprime = array.array("B", b"\x01" * ((ceiling - 1) // 2))
	sieveend = root(ceiling)
