	...
	for (key, words) in anagrams.items():
		for i in range(len(words)):
			for j in range(i + 1, len(words)):
				assignments = {}
				result = max(squares(words[i], words[j], 0, assignments, [False] * 10), result)
	return str(result)


def squares(a, b, index, assignments, usecheck):
	if index == len(a):
		...
		number_two = 0
		for (x, y) in zip(a, b):
			number_one = number_one * 10 + assignments[x]
			number_two = number_two * 10 + assignments[y]
		if common.is_square(number_one) and common.is_square(number_two):
			return max(number_one, number_two)
		else:
			return 0
	
	elif a[index] in assignments:
		return squares(a, b, index + 1, assignments, usecheck)
		...
		for i in range(10):
			if not usecheck[i]:
				...
				result = max(squares(a, b, index + 1, assignments, usecheck), result)
				del assignments[a[index]]
				usecheck[i] = False
		return result


input = (  # 10 strings per line, except final line
	...
	"LIKE", "LIKELY", "ceiling", "LIMITED", "LINE", "LINK", "LIP", "LIST", "LISTEN", "LITERATURE",
